...
public class ConstructController {
	...
	public Map<String, Object> deveDepList(HttpServletRequest request,
		...
		String dep_name = request.getParameter("dep_name");
		sbf.append(" construct_project_dep where 1=1  and constuct_project_dep_company="+constuct_project_dep_company+"");
	 ...
	 public void excelprint(HttpServletRequest request,HttpServletResponse response) throws Exception{
	  ...
	  try {
        ...
        row.setHeight((short) (30*20));
		  ...
		  row2.createCell(4).setCellValue("计划单单号：");
		  row2.setHeight((short) (15*20));
		...
		//在sheet里创建第3行
		  ...
		  row4.createCell(0).setCellValue("工程名称：");
		   row4.createCell(1).setCellValue(construct.getConstruct_project_name());
		    row4.createCell(4).setCellValue("工程地址：");
		  row4.createCell(5).setCellValue(construct.getConstruct_project_addr()); 
         ...
         row5.createCell(5).setCellValue(construct.getConstruct_project_leaderTel());
         ...
         row5.setHeight((short) (15*20));
		  ...
		  row8.createCell(4).setCellValue("送达日期：");
		  if (head.getConstruct_purchase_planDate()!=null && head.getConstruct_purchase_arriveDate()!=null) {
			    ...
			    row8.createCell(5).setCellValue(head.getConstruct_purchase_arriveDate().toString());
		 }else {
			 row8.createCell(1).setCellValue(0);
			  row8.createCell(5).setCellValue(0);
		  ...
		  row.createCell(4).setCellValue("材料复核员：");
		 // sheet.addMergedRegion(new CellRangeAddress(8,8,4,1));
		  row.createCell(3).setCellValue(head.getConstruct_purchase_reviewer());
		// sheet.addMergedRegion(new CellRangeAddress(8,8,7,9));
		 
		  row.createCell(7).setCellValue("材料品牌");
		  ...
		  cellrow6 = row6.createCell(7); 
		  style=sts1(wkb);
		
		  cellrow6.setCellStyle(style);
		  cellrow6.setCellValue("联系方式：");
		  row6.createCell(5).setCellValue(head.getConstruct_purchase_supplierTel());
			...
			for (i = 0; i <rows4.size(); i++){
			   ...
			   for (int j = 0; j < list.size(); j++) {
				...
				/*   Iterator<String> it = myjObject.keys(); 
		           ...
		           sheet.addMergedRegion(new CellRangeAddress(9+i+j+4,9+i+j+4,6,9));
