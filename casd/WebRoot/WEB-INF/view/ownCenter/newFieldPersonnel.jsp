...
<body>
		...
		<div class="easyui-panel" title="外勤申请表" style="width:800px">
			<div style="padding:10px 60px 20px 60px">
	    	 ...
	    	 <tr>
				...
				<select id="seusername"  name="userid" style="width:120px;height: 27px">
				        	<c:forEach items="${userList}" var="user">
								    <option value="${user.userid}">${user.username}</option>
								  </c:forEach>
                            </select>
                </c:if>
					<a href="javascript:void(0)" class="easyui-linkbutton" style="width:50px; background: #d8bfd8"
						onclick="submitForm()">提交</a> <a href="javascript:void(0)"
						class="easyui-linkbutton" style="width:50px;background: #d8bfd8" onclick="clearForm()">返回</a>
   ...
   /* function wen(src) {
	...
	$(function(){
       ...
       //提交单据
	function submitForm(){
    	   var userid=$("#seusername").val();
    	   var username=$('#seusername option:selected').text();//选中的文本("username");
       if(username !=null){
    	   var truthBeTold = confirm('你确定是提交给：'+username);
    	   if (truthBeTold) {
                 } else{
                	 return;
                 };
	 	...
	 	$.post("addFieldPersonne.do?"+$("#jvForm").serialize(),{'userid':userid,'username':username},
				function(data) {
	 		if (data.Success) {		
